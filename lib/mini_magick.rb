  ...
  class Image
    ...
    def [](value)
      ...
      when /^photoshop\:/i
        run_command('identify', '-format', "\"%[#{value}]\"", @path).chop
      when "Profile-icc"
        run_command('identify', '-verbose', @path).match(/Profile-icc: \d+ bytes\n(.+)\n/)[1].to_s.strip
      else
    ...
    def run_command(command, *args)
      ...
      bin_path = File.join([MiniMagick::Image.options[:command_path], command].compact)

      command = "#{bin_path} #{args.join(' ')}"
      output, errors = nil
      ...
      if !errors.blank? && !errors.match(/(wrong data type 7|unknown field with tag)/)
        RAILS_DEFAULT_LOGGER.warn "ImageMagick command (#{command.inspect}) failed: #{{:status_code => $?, :output => output + errors}.inspect}"
        raise MiniMagickError, "ImageMagick command (#{command.inspect}) failed: #{{:status_code => $?, :output => output + errors}.inspect}"
      else
        output
      end
    end
